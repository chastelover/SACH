...
//        readers.add(dblpReader);
        ...
        for (DataReader reader : readers) {
            ...
            try{
                FileWriter write = new FileWriter("/" + reader.toString() + "FHAnalysis.txt");
                BufferedWriter bw = new BufferedWriter(write);

                for(int queryK = 1;queryK <= 30;queryK++){
                    BASIC prunePath = new BASIC(graph, vertexType, edgeType, weight);
                    Set<Integer> rsSet1 = prunePath.query(queryId, queryMPath, queryK);
                    double FH = 0;
                    if(rsSet1 != null)  {
                        FH = prunePath.getFh();
                        System.out.println("queryK: " + queryK  + " 社区值： "+ FH);
                        bw.write(FH + "\n");
                    }
